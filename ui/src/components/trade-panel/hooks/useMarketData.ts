...
export function useMarketData() {
  ...
  const baseToken = selectedMarket
    ? tokens[selectedMarket.base_ticker]
    : undefined;
  ...
  const baseBalance = balances.find(
    (b) => b.token_ticker === baseToken?.ticker,
  );
  ...
  const lastTradePrice =
    recentTrades.length > 0 && recentTrades[0]
      ? recentTrades[0].priceValue
      : null;
  ...
  const priceDecimals = useMemo(
    () =>
      selectedMarket && quoteToken
        ? getDecimalPlaces(selectedMarket.tick_size, quoteToken.decimals)
        : 2,
    [selectedMarket, quoteToken],
  );
