...
export function useOrderLines(
  ...
  const userOrders = useMemo(
    () => Object.values(userOrdersRecord),
    [userOrdersRecord],
  );
      ...
      if (!orderLinesRef.current.has(order.id)) {
        try {
          ...
          line
            ...
            .setTooltip(
              `${sideText} Order: ${order.sizeDisplay} @ ${order.priceDisplay}`,
            )
            .setEditable(true)
            ...
            .onMove(async function (this: { getPrice: () => number }) {
              if (!userAddress) {
                console.warn(
                  "[OrderLines] Cannot modify: user not authenticated",
                );
                return;
              }
              ...
              console.log(
                `[OrderLines] Moving order ${order.id} from ${order.priceValue} to ${newPrice}`,
              );

              // Note: Order modification via chart requires wallet signing
              // This is not yet implemented - show a warning
              console.warn(
                "[OrderLines] Order modification via chart requires wallet signing - not yet implemented",
              );
              alert(
                "Order modification via chart is not yet available. Please use the order panel.",
              );
            })
            .onCancel(async () => {
              if (!userAddress) {
                console.warn(
                  "[OrderLines] Cannot cancel: user not authenticated",
                );
                return;
              }
              ...
              // This is not yet implemented - show a warning
              console.warn(
                "[OrderLines] Order cancellation via chart requires wallet signing - not yet implemented",
              );
              alert(
                "Order cancellation via chart is not yet available. Please use the order panel.",
              );
            });
    ...
    return () => {
      for (const line of orderLines.values()) {
        try {
          ...
          console.warn(
            "[OrderLines] Failed to remove order line on cleanup:",
            err,
          );
        }
      }
